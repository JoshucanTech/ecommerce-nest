...
export class GoogleAdsenseService {
  ...
  async createGoogleAd(advertisementId: string, adData: any) {
    try {
      this.logger.log(`Creating Google AdSense ad for advertisement ID: ${advertisementId}`)
      ...
      this.logger.error(`Error creating Google AdSense ad: ${error.message}`, error.stack)
      throw error
    }
  }
  ...
  async updateGoogleAd(advertisementId: string, adData: any) {
    try {
      ...
      if (!platformRef) {
        throw new Error(`Google AdSense ad reference for advertisement ID ${advertisementId} not found`)
      }
    ...
    try {
      ...
      return {
        ...
        stats: {
          ...
          averagePosition: Number.parseFloat((Math.random() * 5 + 1).toFixed(1)), // 1.0-6.0
        },
      }
    } catch (error) {
      this.logger.error(`Error getting Google AdSense ad stats: ${error.message}`, error.stack)
      throw error
    }
  }
  ...
  async syncAd(advertisementId: string) {
    try {
      ...
      const updatedMetadata = {
        ...
        impressionShare: Number.parseFloat((Math.random() * 0.5 + 0.3).toFixed(2)),
        averagePosition: Number.parseFloat((Math.random() * 5 + 1).toFixed(1)),
        searchImpressionShare: Number.parseFloat((Math.random() * 0.4 + 0.2).toFixed(2)),
      }
      ...
      this.logger.error(`Error syncing Google AdSense ad: ${error.message}`, error.stack)
      throw error
    }
  }
}
