...
export class WhatsappAdService {
  ...
  async createWhatsappAd(advertisementId: string, adData: any) {
    try {
      this.logger.log(
        `Creating WhatsApp ad for advertisement ID: ${advertisementId}`,
      );
      ...
      this.logger.error(
        `Error creating WhatsApp ad: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async updateWhatsappAd(advertisementId: string, adData: any) {
    try {
      this.logger.log(
        `Updating WhatsApp ad for advertisement ID: ${advertisementId}`,
      );
      ...
      if (!platformRef) {
        throw new Error(
          `WhatsApp ad reference for advertisement ID ${advertisementId} not found`,
        );
      }
      ...
      this.logger.error(
        `Error updating WhatsApp ad: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async deleteWhatsappAd(advertisementId: string) {
    try {
      this.logger.log(
        `Deleting WhatsApp ad for advertisement ID: ${advertisementId}`,
      );
      ...
      this.logger.error(
        `Error deleting WhatsApp ad: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async getWhatsappAdStats(advertisementId: string) {
    try {
      this.logger.log(
        `Getting WhatsApp ad stats for advertisement ID: ${advertisementId}`,
      );
      ...
      return {
        ...
        stats: {
          ...
          conversions,
          spend,
          deliveryRate: Number.parseFloat(
          ...
          costPerConversion: Number.parseFloat(
      ...
      this.logger.error(
        `Error getting WhatsApp ad stats: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async syncAd(advertisementId: string) {
    try {
      this.logger.log(
        `Syncing WhatsApp ad for advertisement ID: ${advertisementId}`,
      );
      ...
      const updatedMetadata = {
        ...
        messagesDelivered: Math.floor(
          (Math.random() * 0.2 + 0.8) *
            (Math.floor(Math.random() * 5000) + 500),
        ), // 80-100% delivery rate
      ...
      this.logger.error(
        `Error syncing WhatsApp ad: ${error.message}`,
        error.stack,
      );
      throw error;
    }
