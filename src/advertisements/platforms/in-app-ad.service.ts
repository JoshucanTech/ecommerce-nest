...
export class InAppAdService {
  ...
  async createInAppAd(advertisementId: string, adData: any) {
    try {
      this.logger.log(
        `Creating In-App ad for advertisement ID: ${advertisementId}`,
      );
      ...
      this.logger.error(
        `Error creating In-App ad: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async updateInAppAd(advertisementId: string, adData: any) {
    try {
      this.logger.log(
        `Updating In-App ad for advertisement ID: ${advertisementId}`,
      );
      ...
      if (!platformRef) {
        throw new Error(
          `In-App ad reference for advertisement ID ${advertisementId} not found`,
        );
      }
      ...
      this.logger.error(
        `Error updating In-App ad: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async deleteInAppAd(advertisementId: string) {
    try {
      this.logger.log(
        `Deleting In-App ad for advertisement ID: ${advertisementId}`,
      );
      ...
      this.logger.error(
        `Error deleting In-App ad: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async getInAppAdStats(advertisementId: string) {
    try {
      this.logger.log(
        `Getting In-App ad stats for advertisement ID: ${advertisementId}`,
      );
      ...
      const conversions = analytics.reduce(
          ...
          placementPerformance: {
            banner: {
              ...
              ctr: Number.parseFloat(
                (
                  (Math.floor(clicks * 0.5) / Math.floor(impressions * 0.6)) *
                  100
                ).toFixed(2),
              ),
            },
            interstitial: {
              ...
              ctr: Number.parseFloat(
                (
                  (Math.floor(clicks * 0.4) / Math.floor(impressions * 0.3)) *
                  100
                ).toFixed(2),
              ),
            },
            native: {
              ...
              ctr: Number.parseFloat(
                (
                  (Math.floor(clicks * 0.1) / Math.floor(impressions * 0.1)) *
                  100
                ).toFixed(2),
              ),
            },
      ...
      this.logger.error(
        `Error getting In-App ad stats: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async getRelevantAdsForPage(page: string, userId?: string, limit = 5) {
    try {
      ...
      this.logger.error(
        `Error getting relevant In-App ads: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async syncAd(advertisementId: string) {
    try {
      this.logger.log(
        `Syncing In-App ad for advertisement ID: ${advertisementId}`,
      );
      ...
      this.logger.error(
        `Error syncing In-App ad: ${error.message}`,
        error.stack,
      );
      throw error;
    }
