...
export class AdPlatformsService {
  ...
  async create(createAdPlatformConfigDto: CreateAdPlatformConfigDto, user: any) {
    ...
    if (!advertisement) {
      throw new NotFoundException(`Advertisement with ID ${createAdPlatformConfigDto.advertisementId} not found`)
    }
    ...
    if (user.role !== UserRole.ADMIN && advertisement.vendorId !== user.vendorId) {
      throw new ForbiddenException("You are not authorized to create platform config for this advertisement")
    }
    ...
    if (existingConfig) {
      throw new BadRequestException(`Platform config already exists for this advertisement and platform`)
    }
  ...
  async findAllForAdvertisement(advertisementId: string, user: any) {
    ...
    if (user.role !== UserRole.ADMIN && advertisement.vendorId !== user.vendorId) {
      throw new ForbiddenException("You are not authorized to view platform configs for this advertisement")
    }
  ...
  async findOne(id: string, user: any) {
    ...
    if (user.role !== UserRole.ADMIN && platformConfig.advertisement.vendorId !== user.vendorId) {
      throw new ForbiddenException("You are not authorized to view this platform config")
    }

    return platformConfig
  }

  async update(id: string, updateAdPlatformConfigDto: UpdateAdPlatformConfigDto, user: any) {
    ...
    if (user.role !== UserRole.ADMIN && platformConfig.advertisement.vendorId !== user.vendorId) {
      throw new ForbiddenException("You are not authorized to update this platform config")
    }
  ...
  async remove(id: string, user: any) {
    ...
    if (user.role !== UserRole.ADMIN && platformConfig.advertisement.vendorId !== user.vendorId) {
      throw new ForbiddenException("You are not authorized to delete this platform config")
    }
  ...
  async syncWithPlatform(id: string, user: any) {
    ...
    if (user.role !== UserRole.ADMIN && platformConfig.advertisement.vendorId !== user.vendorId) {
      throw new ForbiddenException("You are not authorized to sync this platform config")
    }
