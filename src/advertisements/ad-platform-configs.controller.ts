...
export class AdPlatformConfigsController {
  ...
  create(@Body() createAdPlatformConfigDto: CreateAdPlatformConfigDto, @CurrentUser() user) {
    return this.adPlatformsService.create(createAdPlatformConfigDto, user)
  }

  @Get("advertisement/:advertisementId")
  @Roles("VENDOR", "ADMIN")
  @ApiOperation({ summary: "Get all platform configurations for an advertisement" })
  @ApiResponse({
  ...
  findAllForAdvertisement(@Param('advertisementId') advertisementId: string, @CurrentUser() user) {
    return this.adPlatformsService.findAllForAdvertisement(advertisementId, user)
  }
  ...
  @ApiParam({ name: "id", description: "Ad Platform Configuration ID" })
  update(@Param('id') id: string, @Body() updateAdPlatformConfigDto: UpdateAdPlatformConfigDto, @CurrentUser() user) {
  ...
  @Post(":id/sync")
  @Roles("VENDOR", "ADMIN")
  @ApiOperation({ summary: "Sync ad platform configuration with external platform" })
  @ApiResponse({
    status: HttpStatus.OK,
