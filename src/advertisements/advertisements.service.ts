...
export class AdvertisementsService {
  ...
  async create(createAdvertisementDto: CreateAdvertisementDto, user: any, config?: JSON) {
    ...
    const { platforms, targeting, title, products, ...adData } = createAdvertisementDto
    ...
    if (products && products.length > adData.maxProducts) {
      throw new BadRequestException(`This advertisement can only hold up to ${adData.maxProducts} products`)
    }
  ...
  async update(id: string, updateAdvertisementDto: UpdateAdvertisementDto, user: any, config?: JSON, name?: string) {
    ...
    // Extract platforms and targeting from DTO
    const { platforms, products, targeting, ...adData } = updateAdvertisementDto
    ...
    if (adData.maxProducts !== undefined) {
      ...
      if (adData.maxProducts < currentProductCount) {
        throw new BadRequestException(`Cannot reduce maxProducts below current product count (${currentProductCount})`)
      }
    }
  ...
  private async addProductsToAdvertisement(advertisementId: string, products: any[]) {
    ...
    if (products.length > advertisement.maxProducts) {
      throw new BadRequestException(`This advertisement can only hold up to ${advertisement.maxProducts} products`)
    }
    ...
    const productAssociations = products.map((product, index) => ({
      ...
      displayOrder: product.displayOrder !== undefined ? product.displayOrder : index,
  ...
  async manageAdvertisementProducts(id: string, productData: any, user: any) {
    ...
    switch (action) {
      case "add":
        ...
        if (advertisement.products.length + products.length > advertisement.maxProducts) {
  ...
  async getFeaturedProducts(params: { page: number; limit: number; categoryId?: string }) {
    ...
    const transformedProducts = featuredProducts.map((item) => ({
      ...
      images: item.customImageUrl ? [item.customImageUrl, ...item.product.images] : item.product.images,
